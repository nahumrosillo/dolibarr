...
/**
 * \file    dolipurge/admin/setup.php
 * \ingroup dolipurge
 * \brief   DoliPurge setup page.
 */
...
require_once '../lib/dolipurge.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "dolipurge@dolipurge"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('dolipurgesetup', 'globalsetup'));
...
$formSetup->newItem('DOLIPURGE_MYPARAM5')->setAsYesNo();
...
if($action == 'update') {
	...
	$consent = $formSetup->items['DOLIPURGE_MYPARAM5']->fieldValue == "1";

	// Pruge all data from this tables
	if ($consent) {
		...
		if ($error) {
			setEventMessage('Se han encontrado '.$error. ' al purgar los datos.', 'errors');
		} else {
			setEventMessage('Data purged');
		}
	}
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/dolipurge/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=dolipurge-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'DOLIPURGE_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'DOLIPURGE_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "DoliPurgeSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-dolipurge page-admin');
...
$head = dolipurgeAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "dolipurge@dolipurge");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("DoliPurgeSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'DOLIPURGE_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'DOLIPURGE_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';

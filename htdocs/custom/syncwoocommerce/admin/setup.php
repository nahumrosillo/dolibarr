...
/**
 * \file    syncwoocommerce/admin/setup.php
 * \ingroup syncwoocommerce
 * \brief   SyncWoocommerce setup page.
 */
...
require_once '../lib/syncwoocommerce.lib.php';
//require_once "../class/myclass.class.php";
// Require Entrepot class
dol_include_once('/product/class/product.class.php');

// Translations
$langs->loadLangs(array("admin", "syncwoocommerce@syncwoocommerce"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('syncwoocommercesetup', 'globalsetup'));
...
//$item = $formSetup->newItem('SYNCWOOCOMMERCE_MYPARAM1');
//$item->fieldOverride = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
//$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
//$item = $formSetup->newItem('SYNCWOOCOMMERCE_MYPARAM2');
...
$item = $formSetup->newItem('SYNCWOOCOMMERCE_CLOUDFLARE_ACCOUNT_ID');
$item->fieldAttr['class'] = 'flat minwidth400';
...
//$item = $formSetup->newItem('SYNCWOOCOMMERCE_MYPARAM10');
//$item->setAsMultiSelect($TField);
//$item->helpText = $langs->transnoentities('SYNCWOOCOMMERCE_MYPARAM10');

// Setup conf for a category selection
//$formSetup->newItem('SYNCWOOCOMMERCE_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf SYNCWOOCOMMERCE_MYPARAM10
//$item = $formSetup->newItem('SYNCWOOCOMMERCE_MYPARAM10');
//$item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir . "core/modules/syncwoocommerce/doc/pdf_" . $modele . "_" . strtolower($tmpobjectkey) . ".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: " . DOL_URL_ROOT . "/document.php?modulepart=syncwoocommerce-" . strtolower($tmpobjectkey) . "&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'SYNCWOOCOMMERCE_' . strtoupper($tmpobjectkey) . "_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'SYNCWOOCOMMERCE_' . strtoupper($tmpobjectkey) . '_ADDON_PDF';
...
$help_url = '';
$title = "SyncWoocommerceSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-syncwoocommerce page-admin');
...
$head = syncwoocommerceAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "syncwoocommerce@syncwoocommerce");

// Setup page goes here
echo '<span class="opacitymedium">' . $langs->trans("SyncWoocommerceSetupPage") . '</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_' . strtolower($myTmpObjectKey) . '_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'SYNCWOOCOMMERCE_' . strtoupper($myTmpObjectKey) . '_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir . '/' . $file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'SYNCWOOCOMMERCE_' . strtoupper($myTmpObjectKey) . '_ADDON_PDF';
